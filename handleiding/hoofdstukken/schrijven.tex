...
\HL
\NC Spectral range                  \NC 0.1 -- \quantity{1.5}{mm} \NR
...
Je bronnen bijhouden tijdens het schrijven van een stukje kan een hel zijn. Welke bronnen heb ik allemaal gebruikt? Waar stond dit ook al weer in? Hoe moet ik precies verwijzen? Hoe moet ik mijn bronnen opgeven? Een veelgebruikte oplossing in combinatie met \TEX\ is Bib\TEX.\footnote{Goh, waar zou de naam vandaan komen?}

Met Bib\TEX\ kun je een database bijhouden van al je bronnen, aan elke bron een label toekennen en met dat label naar je bronnen verwijzen. Ook helpt Bib\TEX\ bij het opmaken van al je gebruikte bronnen aan het einde van je tekst.

Een Bib\TEX\ database is platte tekst in een bestand eindigend op \type{.bib}. De database bestaat uit bronnen van een bepaald type, \emph{\en entries} genaamd, met daarin alle gegevens van de bron, de \emph{\en fields}. \cite[title][Griffiths:2008] van \cite[author][Griffiths:2008] geef je bijvoorbeeld zo op.
...
Dit is een entry van het type \type{book}, wat we opgeven achter het apenstaartje. \type{Griffiths:2008} is het label waarmee we naar deze entry kunnen verwijzen. De overige regels bestaan uit velden met een naam en hun waarde tussen accolades. Wil je verwijzen naar een bepaald hoofdstuk in een boek of bepaalde pagina's, dan kun je het type \type{inbook} gebruiken.
...
Natuurlijk wil je niet alleen maar naar boeken kunnen verwijzen. Een artikel begint met \type{@article}.
...
Er zijn nog veel meer \emph{\en entry types} en \emph{field names}. Een overzicht kun je vinden in \in{appendix}[app:bibtex]. Bij bepaalde typen zijn sommige velden verplicht en andere niet. Dit verschilt per type. Let hier op.

Om te verwijzen naar een bron uit je Bib\TEX\ database gebruik je \tex{cite}. In zijn simpelste form geef je \tex{cite} het label van je bron. De rest doet \TEX.

\todo{Lege bronnen? => Verkeerd gespelde labels}

\todo{Verwijzen naar excursie en reference}

\todo{\tex{placepublications}}
