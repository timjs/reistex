...
Commando's in \TEX\ worden voorafgegaan met een @\@. De tekst waar het commando op werkt zetten we tussen @{ }@. We kunnen bijvoorbeeld een nieuw hoofdstuk aanmaken over Marie Curie met @\chapter{Marie Curie}@. Dit ene commando kan meerdere acties tot gevolg hebben. Bijvoorbeeld:
...
Soms wil je dat een commando op meerdere regels of alinea's werkt. Hiervoor zetten we de tekst tussen een @start/stop@ paar. Een voorbeeld hiervan is @\startitemize@ en @\stopitemize@, wat we gaan gebruiken om opsommingslijsten te maken.

\section{Structuur}

\subsection{Starten en stoppen}

Laten we beginnen met een minimaal reisverslagstukje. Open je favoriete tekstbewerker, tik het volgende en sla het op onder de naam @mijnstukje.tex@.
...
Met @\startcomponent@ geven we aan dat we een nieuw stukje beginnen. Het stukje heeft de naam die er achter staat\seedocumentation{1}{2.2 \& 2.3}. De volgende regel laadt de opmaak, die gedefinieerd staat in het bestand @travellayout.tex@. We sluiten het stukje af met @\stopcomponent@. Elke @\start@ moet afgesloten worden met een @\stop@. Tussen @\startcomponent@ en @\stopcomponent@ tikken we de inhoud van het stukje. In dit geval is dit alleen de tekst \quote{Dit is mijn eerste stukje.}.
...
Een nieuw hoofdstuk maak je in \CONTEXT\ met het commando @\chapter@. Maar wij willen graag dat jouw naam en het datum van het bezoek bij het hoofdstuk komen te staan. Hiervoor gebruiken we een @\start/stopchapter@-paar waar we deze extra informatie aan meegeven.
...
Zoals je ziet geven we \quote{standaard informatie} en \quote{extra informatie} op in twee verschillende lijsten, omgeven met @[ ]@. Voor de datum gebruiken we het commando @\date@, waar we door middel van nog een lijst dag, maand en jaar opgeven. Let goed op de haakjes en plekken waar je komma's moet zetten. Tussen de @\startchapter@ en @\stopchapter@ komt de tekst van jou hoofdstuk te staan.

De rest van je stukje kun je indelen met subkopjes. Deze heten in \CONTEXT\ @\section@, @\subsection@, @\subsubsection@ enzovoorts\seedocumentation{5}{8.2}. Als argument geef je de titel van het kopje.
...
Forceer \TEX\ \emph{nooit} om binnen een alinea regels af te breken. Dit levert niet alleen een lelijk uitgelijnde tekst op. Het zorgt er ook voor dat \TEX\ meer moeite heeft om je alinea af te breken en over pagina's te verdelen. Bedenk dus goed waar je een nieuwe alinea begint en gebruik geen \emph{subalinea's} of iets dergelijks. Het in \LATEX\ veelgebruikte commando @\\@ is in \CONTEXT\ niet eens gedefinieerd!
\stopframedtext

\subsection{Escapes}

De tekens \%, \#, \$, \&, \{ en \} moeten worden voorafgegaan door een \letterbackslash\ omdat deze een speciale betekenis hebben in \TEX. Om de \letterbackslash\ zelf te zetten gebruik je @\letterbackslash@\seedocumentation{3}{1.6}.

Ook \~ is een speciaal teken dat je niet zomaar kunt gebruiken. Met een \~ kun je namelijk een spatie afdwingen waartussen geen regeleinde kan komen. Erg handig in mevr.~M.~Curie bijvoorbeeld! Een \~ zelf zet je met @\~@.
...
Het belangrijkste dat je kunt doen om een stuk tekst op te laten vallen\seedocumentation{31.7}{5.6}\ is die \emph{accentueren}. Dit kan met @\emph@. Daarnaast kun je afkortingen, die meestal in hoofdletters worden gezet, minder SCHREEUWERIG maken met @\cap@. Dat ziet er dan \cap{rustiger} uit\seedocumentation{31.6}{5.7}.\footnote{Voor meer over afkortingen, zie \in{sectie}[sec:afkortingen]} Afbreekstreepjes zijn gewone -, gedachtestreepjes worden gemaakt met twee streepjes --. Dit langere streepje is ook erg handig tussen getallen: 1997--2002. Soms wil je een zin afsluiten met drie puntjes\dots\ Dat kan met @\dots@. Een \URL\ kun je zetten met @\online@.
...
%Soms wil je chemische formules zetten zoals H\low{2}O.\seedocumentation{37.6}{4.9}\ Dan heb subscript of superscript nodig. Hiervoor zijn de commando's @\low@ en @\high@ beschikbaar. Wanneer je iets tegelijk laag en hoog wilt zetten, zoals \lohi{11}{23}Na, kun je @\lohi@ gebruiken.

%Voor sub- en superscript in formules zie \in{sectie}[sec:formules].

\subsection{Aanhalingen}

Je kunt aanhalingen maken in de tekst met @\quote@\seedocumentation{--}{10.8}. @\quote@ is slim genoeg om voor jou de juiste aanhalingstekens te kiezen als je binnen @\quote@ nog een @\quote@ maakt. \quote{A question that sometimes drives me hazy: \quote{Am I or are the others crazy?}} (--Albert Einstein) Langere aanhalingen kun je tussen @\startquote@ en @\stopquote@ zetten.

\startframedtext
Let op dat je bij het aanhalen van stukken tekst gebruik maakt van het commando @\quote@. Gebruik dus nooit de \quote{domme} aanhalingstekens op je toetsenbord.
\stopframedtext

\subsection{Voetnoten}

Fysici zijn dol op voetnoten. In je fysica-stukje wil je er misschien niet te veel gebruiken, maar in je cultuurstukje kun je natuurlijk helemaal los gaan! Een voetnoot maak je met @\footnote@\seedocumentation{14}{4.18}.\footnote{Ze krijgen dan een nummer en worden onder aan de pagina geplaatst.}

\subsubject{Voorbeeld}

In onderstaand voorbeeld kun je zien hoe alle bovenstaande commando's werken en het resultaat daarvan. Let er op dat @\emph@ weet dat de omvattende tekst cursief is en het benadrukte woord roman zet. Merk ook op dat \TEX\ de regel afbreekt na de -- en niet tussen het getal 212 en \cap{v.Chr.}. Dat komt door het gebruik van \~.
...
Opsommingslijsten kun je zetten tussen @\startitemize@ en @\stopitemize@\seedocumentation{6}{10.6}. Elk lijstonderdeel begint met @\item@. Laten we maar meteen een voorbeeld geven over elementaire deeltjes.
...
Zoals je ziet kun je ook lijsten binnen lijsten zetten. Wanneer je de onderdelen van een lijst wilt nummeren geef je de optie @[n]@ mee. Bijvoorbeeld:
...
\getbuffer
Dit zijn de fundamentele natuurkrachten in aflopende\dots\ Wacht even. Volgens mij zijn we er eentje vergeten. Het is dan wel het buitenbeentje in de theorie, maar toch. Gelukkig kunnen we altijd nog een item toevoegen aan de lijst met @[continue]@.
...
Zo, dat is beter. Soms wil je een lijst maken met kopjes. In plaats van @\item@ gebruik je dan @\head@.
...
Let er wel op dat je na de kop een regel open laat, anders krijg je een hele lange kop. Je kunt een genummerde opsomming met kopjes maken door, je raadt het al, weer @[n]@ toe te voegen. Oh, nu ben ik bijna vergeten je te laten zien hoe die kopjes er dan uit zien\dots

\getbuffer

\section{Figuren}

Tabellen en figuren zijn zogenaamde \emph{blokken}\seedocumentation{10}{12.2}. \TEX\ zet ze daar neer, waar nog plaats is op de pagina. Ze hebben vaak een bijschrift en worden automatisch genummerd. Eerst even een voorbeeld van een welbekend logo.\footnote{Zie meer over @\infull@ in \in{sectie}[sec:synoniemen]}
...
Om een figuur te plaatsen gebruiken we @\placefigure@. Dit commando heeft maar liefst vier argumenten. Twee tussen blokhaakjes (die optioneel zijn) en twee tussen accolades.

Met het eerste argument tussen blokhaakjes hoef je niets te doen, die kun je dus leeg laten. Het tweede argument bevat een label. Met dit label kun je later naar je figuur verwijzen. Je weet immers niet welk nummer het figuur heeft en op welke pagina die komt te staan. \CONTEXT\ zoekt dit dan automatisch voor je uit. Verzin wel duidelijke labels! Andere mensen vinden \quote{plaatje} of \quote{instituut} misschien ook wel een goed label en dan raakt \TEX\ in de war. Als je geen verwijzingen wilt maken, dan kun je beide rechte haakjes van @\placefigure@ leeg laten. Meer informatie over verwijzingen vind je in \in{sectie}[sec:verwijzingen].

Het derde en vierde argument bevatten het bijschrift en het figuur zelf. Een figuur laden we in met @\externalfigure@\seedocumentation{10}{13.2 \& 13.3}. Als eerste geven we de bestandsnaam van het figuur op. Hier is dat @marielogo@. Hierbij hoef je \emph{geen extensie} op te geven, \CONTEXT\ zoekt zelf uit wat voor type figuur het is. Daarnaast melden we hoe groot het figuur moet zijn. Je kunt kiezen uit @large@, @medium@ en @small@. Soms heb je plaatjes die je over de hele tekstbreedte wilt zetten, gebruik dan @broad@. Voor plaatjes die smal zijn gebruik je (je raadt het al) @narrow@. Er zijn nog meer van dit soort groottes gedefinieerd, dus schrik niet als de cie iets anders kiest. Het is uiteindelijk hun taak om het verslag netjes op te maken.
...
Tabellen in \TEX\ zijn iets ingewikkelder. In \CONTEXT\ maak je een tabel met @\starttabulate@ en @\stoptabulate@\seedocumentation{--}{4.12}. Als eerste moet je \CONTEXT\ vertellen hoeveel kolommen je tabel heeft en hoe ze uitgelijnd zijn. Dit geef je tussen blokhaakjes mee aan @\starttabulate@. Voor de uitlijning van een kolom kun je kiezen tussen @l@ ({\en left}), @c@ ({\en center}), @r@ ({\en right}) of @p@ ({\en paragraph}). De eerste drie spreken voor zich. Met de laatste optie kun je meer dan één regel tekst in een cel kwijt. Deze \emph{formaataanduidingen} scheid je met een kolomscheider, de @|@.

Tussen @\starttabulate@ en @\stoptabulate@ geef je de inhoud van de tabel. Een nieuwe kolom begin je met @\NC@ ({\en New Column}). Een nieuwe rij met @\NR@ ({\en New Row}). Je kunt ook horizontale lijnen trekken met @\HL@ ({\en Horizontal Line}). Een eenvoudige tabel met twee kolommen, de eerste links uitgelijnd en de tweede rechts, ziet er dan als volgt uit.\footnote{Voor meer info over de eenheden in de tweede kolom zie \in{sectie}[sec:eenheden].}
...
\HL
\NC Spectral range                  \NC \unit{0.1 -- 1.5 mm} \NR
\NC Micro-pulse frequency           \NC \unit{3 GHz}         \NR
\NC Micro-pulse duration            \NC \unit{10 -- 100 ps}  \NR
\NC Macro-pulse duration            \NC \unit{10 -- 15 μs}   \NR
\NC Repetition rate of macro-pulses \NC \unit{10 Hz}         \NR
\NC Electron beam energy            \NC \unit{10 -- 15 MeV}  \NR
\NC Undulator period                \NC \unit{110 mm}        \NR
\NC Number of undulator periods     \NC \unit{40 }           \NR
\NC Undulator parameter Krms        \NC \unit{0.7 -- 3.4 }   \NR
\NC Optical cavity length           \NC \unit{7.5 m}         \NR
\HL
\stoptabulate
\stopbuffer
\typebuffer
...
Zoals je ziet gaat het plaatsen van een tabel hetzelfde als bij een figuur. Ook hier kunnen we een label en een bijschrift meegeven, maar in plaats van @\placefigure@ gebruiken we nu @\placetable@, en we geven nu natuurlijk een tabel als laatste argument in plaats van een extern figuur.\footnote{Er staan impliciete haakjes om @\start/stoptabulate@.}
...
De tweede modus om formules in te zetten is in \emph{\en display mode}. Deze modus gebruiken we om formules op een eigen regel te plaatsen. Hiervoor zetten we de formule tussen @\startformula@ en @\stopformula@.
...
Zoals je ziet hebben we de formule voor $\gamma$ laten nummeren en een label gegeven. Dit hebben we gedaan met weer een nieuwe @\place@-variant, namelijk @\placeformula@.\footnote{In tegenstelling tot @\placefigure@ en @\placetable@ heeft @\placeformula@ maar een paar blokhaakjes nodig!}

Soms wil je een een paar formules netjes onder elkaar zetten. Dit doen we op dezelfde manier waarop we tabellen maken, dus met @\NC@ en @\NR@. We hebben wel een extra commandopaar nodig, namelijk @\start/stopalign@.
...
Enkele voorbeelden van \quote{ingewikkeldere} wiskunde. Let op @\total@ om een \emph{rechte d} te zetten en @\vec@ voor vectoren.%Je kunt meerdere formules groeperen tussen @\startformulas@ en @\stopformulas@. \CONTEXT\ kiest dan een mooie uitlijning voor je.
...
Hmm, volgens mij hebben scheikundigen de letters toch liever rechtop\dots\ In \CONTEXT\ kun je een chemische formule zetten met @\chemical@. Om moleculen te scheiden gebruik je komma's, de rest spreekt voor zich.
...
Zo, nu zijn de scheikundigen wel tevreden! @\chemical@ kun je ook in de lopende tekst gebruiken en ook om een enkel molecuul of ion te zetten. @\chemical{C_2O_4^{2-}}@ geeft netjes \chemical{C_2O_4^{2-}}. Met dit commando kun je zelfs hele chemische structuren tekenen! Het gaat iets te ver dat hier ook allemaal uit te leggen, meer informatie kun je vinden in \from[chemical].

\subsection[sec:eenheden]{Eenheden}

Vaak zul je in je tekst natuurkundige grootheden gebruiken zoals \unit{6,626068e-34 m^2kg/s} en \unit{2,99792458 m/s}. Laten we even kijken wat er gebeurt wanneer we dit in wiskundemodus zetten.
...
De oplossing komt in de vorm van het commando @\quantity@. Je geeft het commando twee argumenten: een getal en een eenheid. @\quantity@ zorgt dan voor twee\footnote{Twee is duidelijk het magische getal hier.} dingen: de eenheid blijft \emph{altijd} netjes rechtop en het getal wordt \emph{nooit} gescheiden van de eenheid. Dat betekend dus nooit meer een getal aan het einde van de regel en een losse \quote{m} op de volgende!

Zoals je nu al vermoedt hoef je @\quantity@ niet allen in wiskundemodus te gebruiken. Vooral de eigenschap dat getal en eenheid bij elkaar blijven zijn juist handig in de lopende tekst. Als bonus\footnote{Hier komt de magie om de hoek kijken.} kun je de handige \cap{e}-syntaxis \footnote{Wel bekend van welke rekenmachine dan ook.} gebruiken om tienmachten aan te geven. Vergelijk dit met bovenstaande.

\startbuffer
\startformulas
\startformula
\hbar = \unit{6,626068e-34 m^2kg/s}
\stopformula
\startformula
c = \unit{2,99792458 m/s}
\stopformula
\stopformulas
\stopbuffer
\typebuffer
...
Zoals we eerder al aanstipten kun je aan tabellen, figuren en formules labels meegeven. Die labels kun je gebruiken om naar het object te verwijzen. Dit doen we met @\in@\seedocumentation{25}{9.5}. Wanneer we aan @\in@ een label meegeven (tussen blokhaakjes), zoekt \CONTEXT\ op welk nummer het label heeft en wordt dat in de tekst ingevoegd. Soms wil je ook de pagina weten waarop het label staat. Dit kun je invoegen met @\at@.
...
Je ziet dat we niet alleen het label meegeven maar ook nog een extra stukje tekst. @\in@ en @\at@ zorgen er voor dat de regel niet opeens afgebroken wordt tussen het woord \emph{figuur} en het nummer.

\section[sec:synoniemen,sec:afkortingen]{Afkortingen}

Fysici zijn lui. Fysici korten alles af. Het komt vaak genoeg voor dat je meerdere keren in je stukje dezelfde afkorting moet gebruiken. Pfff, en dan elke keer weer @\cap@ gebruiken, dat is gewoon veel te veel moeite\dots

Gelukkig kan \CONTEXT\ goed omgaan met zogenaamde \emph{synoniemen} (\emph{\en synonyms})\seedocumentation{24}{9.2}. We hoeven een afkorting maar één keer te definiëren, en daarna kun je hem steeds weer opnieuw gebruiken. We zijn er onderweg al twee tegengekomen. Namelijk \MC\ en \FLARE. Ze zijn gedefinieerd met @\abbreviation@.

\typebuffer[example:abbreviations]

Na deze definities zijn twee nieuwe commando's beschikbaar, namelijk @\MC@ en @\FLARE@. Deze kun je nu gebruiken om de afkortingen in je tekst te zetten. Daarnaast kun je met @\infull@ de betekenis van de afkorting oproepen. Laten we eerst eens het volgende voorbeeld bekijken.
...
In @travellayout.tex@ hebben we een paar extra afkortingen voorgedefinieerd. Je kunt de volgende afkortingen allemaal gebruiken:
...
Een \BIBTEX-database is platte tekst in een bestand eindigend op @.bib@. De database bestaat uit bronnen (\emph{\en entries} genaamd) van een bepaald type met daarin alle gegevens van de bron in velden (\emph{\en fields}). \emph{Introduction to Electrodynamics} van Griffiths geef je bijvoorbeeld zo op.

\typefile{literatuur/griffiths.bib}

Dit is een bron van het type @book@, wat we opgeven achter het apenstaartje. @Griffiths:2008@ is het label waarmee we naar deze bron kunnen verwijzen. De overige regels bestaan uit velden met hun waarde tussen accolades. Let op de komma's aan het eind van elk veld!\footnote{De komma achter het laatste veld is niet verplicht, maar aangezien je ze anders toch vergeet\dots}

Heb je specifieke pagina's of hoofdstukken uit een boek gebruikt, dan kun je het type @inbook@ gebruiken.

\typefile{literatuur/kuhn.bib}

Hier zie je dat, wanneer een boek meerdere auteurs heeft, je die opgeeft in \emph{hetzelfde} veld gescheiden door @and@. Verder maakt het niet uit of je de volledige namen van de auteurs kent of niet. Ook kun je eerst de achternaam opgeven en daarna de initialen of voornamen, zolang er maar een komma tussen staat.
...
Om te verwijzen naar een bron uit je \BIBTEX\ database gebruik je @\cite@. In zijn simpelste form geef je @\cite@ het label van je bron tussen blokhaken, bijvoorbeeld @\cite[Griffiths:2008]@. Dit geeft \cite[Griffiths:2008]. Meer info over bibliografieën kun je vinden in \from[bib].

\BIBTEX\ nummert al je bronnen en zet ze, zoals gebruikelijk binnen de natuurkunde, tussen blokhaken.
@travellayout.tex@ zorgt er zelf voor dat aan het einde van je hoofdstuk een bronnenlijst verschijn!
%Het enige dat jij nog moet doen is @\placepublications@ aan het einde van je stukje te zetten!
...
%De bronnen die je hebt gebruikt plaats je in een bibliografie aan het einde van je stukje tussen @\startbibliography@ en @\stopbibliography@. Net als bij opsommingen wordt elk onderdeel voorafgegaan met @\item@. Natuurlijk is het handig als je naar je bronnen kunt verwijzen. Aan @\item@ kun je (net als bij figuren en tabellen) een label toevoegen. Je kunt er dan naar verwijzen met @\inbib@.
