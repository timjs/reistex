...
\TEX\ is een opmaaktaal speciaal voor het zetten van documenten. We schrijven een tekst in een @.tex@-bestand, ook wel de \emph{bron} genoemd. Dit is gewoon platte tekst, die je kunt bewerken in je lievelingstekstbewerker. Vervolgens laten we dit door \TEX\ omzetten naar een \PDF, het \emph{compileren}.
...
Laten we nu een mooi kopje maken met de titel van je stukje, de datum van het bezoek en je naam. Dit doen we met @\report@.

\starttyping
\report{Titel}{\date[d=1,m=1,y=2011]}{Naam}
\stoptyping

Zoals je ziet bevat het eerste argument de titel van je stukje. In het tweede argument gebruiken we @\date@, waar we door middel van een lijst dag, maand en jaar opgeven. Het derde argument bevat je naam.
...
De tekens \%, \#, \$, \&, \{ en \} moeten worden voorafgegaan door een \backslash\ omdat deze een speciale betekenis hebben in \TEX. Om de \backslash\ zelf te zetten gebruik je @\backslash@\seedocumentation{3}{1.6}.

Ook \sim\ is een speciaal teken dat je niet zomaar kunt gebruiken. Met een \sim\ kun je namelijk een spatie afdwingen waartussen geen regeleinde kan komen. Erg handig in mevr.~M.~Curie bijvoorbeeld! \sim\ zelf zet je met @\sim@.\footnote{Om de spatie na \sim\ te behouden tik je @\sim\@ net als bij synoniemen (zie \in{sectie}[sec:synoniemen]).}
...
In onderstaand voorbeeld kun je zien hoe alle bovenstaande commando's werken en het resultaat daarvan. Let er op dat @\emph@ weet dat de omvattende tekst cursief is en het benadrukte woord roman zet.

\startbuffer
\startquote
Archimedes was een Griekse wis- en natuurkundige die leefde van
287 -- 212~\cap{v.Chr.} in Syracuse op Sicilië.\footnote{Bron:
\url{http://www.math4all.nl/Wiskundegeschiedenis/Wiskundigen/Archimedes.html}}
...
\getbuffer
Dit zijn de fundamentele natuurkrachten in aflopende\periods\ Wacht even. Volgens mij zijn we er eentje vergeten. Het is dan wel het buitenbeentje in de theorie, maar toch. Gelukkig kunnen we altijd nog een item toevoegen aan de lijst met @[continue]@.
...
Let er wel op dat je na de kop een regel open laat, anders krijg je een hele lange kop. Je kunt een genummerde opsomming met kopjes maken door, je raadt het al, weer @[n]@ toe te voegen. Oh, nu ben ik bijna vergeten je te laten zien hoe die kopjes er dan uit zien\periods
...
Zoals je ziet gaat het plaatsen van een tabel hetzelfde als bij een figuur. Ook hier kunnen we een label en een bijschrift meegeven, maar in plaats van @\placefigure@ gebruiken we nu @\placetable@, en we geven nu natuurlijk een tabel als laatste argument in plaats van een extern figuur.
...
Enkele voorbeelden van \quote{ingewikkeldere} wiskunde. Let op @\d@ om een \emph{rechte d} te zetten en @\vec@ voor vectoren.%Je kunt meerdere formules groeperen tussen @\startformulas@ en @\stopformulas@. \CONTEXT\ kiest dan een mooie uitlijning voor je.

\startbuffer
\startformula
f(x) = \left| \int_0^1 (x^2 - 3x)    \,\d x \right|
     + \left| \int_1^2 (x^2 -5x + 6) \,\d x \right|
\stopformula
\stopbuffer
\typebuffer
\getbuffer
...
Hmm, volgens mij hebben scheikundigen de letters toch liever rechtop\periods\ In \CONTEXT\ kun je een chemische formule zetten met @\chemical@. Om moleculen te scheiden gebruik je komma's, de rest spreekt voor zich.
...
Fysici zijn lui. Fysici korten alles af. Het komt vaak genoeg voor dat je meerdere keren in je stukje dezelfde afkorting moet gebruiken. Pfff, en dan elke keer weer @\cap@ gebruiken, dat is gewoon veel te veel moeite\periods
...
Om te verwijzen naar een bron uit je \BIBTEX\ database gebruik je @\cite@. In zijn simpelste form geef je @\cite@ het label van je bron tussen blokhaken, bijvoorbeeld @\cite[Griffiths:2008]@. Dit geeft \cite[Griffiths:2008]. Meer info over bibliografieën kun je vinden op \from[bibman].

\BIBTEX\ nummert al je bronnen en zet ze, zoals gebruikelijk binnen de natuurkunde, tussen blokhaken. Het enige dat jij nog moet doen is @\placepublications@ aan het einde van je stukje te zetten!
...
%\item \url{http://en.wikipedia.org/wiki/Electron_rest_mass}
%\stopbibliography
%\stopbuffer
%\typebuffer
