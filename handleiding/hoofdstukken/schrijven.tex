...
\startframedtext
Let op dat je je stukje opslaat in \emph{Unicode} codering (ook wel \UTF-8 genoemd). De manier om dit te doen verschilt per tekstbewerker. Soms kun je dit aangeven in een menu(-item) met de naam \emph{Codering}, soms kun je dit aangeven in het venster \emph{Opslaan als\dots} als extra optie.
\stopframedtext
...
Om te verwijzen naar een bron uit je \BIBTEX\ database gebruik je \tex{cite}. In zijn simpelste form geef je \tex{cite} het label van je bron tussen blokhaken, bijvoorbeeld \type{\cite[Griffiths:2008]}. Dit geeft \cite[Griffiths:2008]. Meer info over bibliografieën kun je vinden op \from[bib].
