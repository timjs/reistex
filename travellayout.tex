...
%M   [packed]
%M
%M \abbreviation{SMCR}{Stichting Marie Curie Reizen}
%M
%M \define[1]\emph
%M   {{\em #1}}
%M
...
%D \module
%D   [file=travellayout,
%D    version=22 sep 2011,
%D    title=Travel Layout,
%D    subtitle=Omgeving voor reisverslagen en readers,
%D    author=Tim Steenvoorden,
%D    date=\currentdate,
%D    copyright=\infull{SMCR}]
%D 
%D \completecontent
...
unitwave    := (origin for x=0 step 0.1 until waveWidth:
                 .. (x,x*sin(x)) 
...
draw unitwave xscaled CutSpace yscaled (LineHeight/2) shifted (OverlayWidth,0);
setbounds currentpicture to boundingbox OverlayBox;
\stopuniqueMPgraphic 

\startuniqueMPgraphic{travellayout::leftwave}
pickup pencircle scaled 1pt; 
draw unitwave xscaled (-1*CutSpace) yscaled (LineHeight/2);
draw origin -- (OverlayWidth,0);
setbounds currentpicture to boundingbox OverlayBox;
\stopuniqueMPgraphic 
...
%D \section{Informatie}
%D

\usemodule[database]

\defineseparatedlist[information]
  [separator={:},
   before={\starttabulate[|le|p|]},
   first=\NC,
   right=\NC,
   last=\NR,
   after=\stoptabulate]
...
\setupbibliography[each]
  [packed]
  [symbol=n,
   left={[},
   right={]},
   stopper=,
   itemalign=flushright,%FIXME: Lijkt niet te werken
   width=2.5em]

\definereferenceformat[inbib]
  [left={[},
   right={]}]
...
%D \section{Afkortingen, eenheden en \cap{url}'s}
%D
