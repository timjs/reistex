...
\report{Lecture from Prof.~Katsnelson}{\date[d=18,m=9,y=2009]}{Sjoerd Verhagen}
...
\placefigure[outer][symmetrie]
  ...
  {\externalfigure[symmetrie][width=0.4\textwidth]}

Not every combination of the 14 Bravais lattices with the 32 point groups constitutes a unique space group. Since a detailed description about the point group and Bravais lattice counting would be out of the scope of Prof. Katsnelson his talk, I will instead reveal the seventeen possible space groups in 2\cap{d}. These groups give all possibilities for texture art on surfaces and have all been in use since the ancient times. Still, all these patterns are in extensive use throughout the muslim world \cite[Grunbaum:1990].
...
To prove there are only seventeen possible 2\cap{d} lattice symmetry groups one can employ Euler Characteristics. Euler Characteristics in 3\cap{d} states the number of corners minus the number of edges plus the number of faces always equals 2. This brought down to two dimensions demands the features of a 2\cap{d} lattice group to sum up to 2 via the following value assignment:
...
In 1914, the Russian doctor Mandelstam returned from Germany to the Moscow State University in his homeland Russia. His field was optics and radio physics. Independent from \cap{c}.\cap{v}. Raman, the \cap{us} based Indian physicist, he discovered the inelastic scattering of light a week before Ramans eyes first witnessed the effect. But Mandelstam, being a theoretician, spent too much time on theoretical considerations on the new phenomenon to be the first to publish. As a result, the effect now beares Raman's name. Also, in 1930, Raman received the Nobel prize of physics for this effect which greatly influences chemistry being a significant tool for analyzing the composition of liquids, gases and solids. 
